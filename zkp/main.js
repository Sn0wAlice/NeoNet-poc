...
class ZKP {
  ...
  generateValidProof() {
    ...
    return {
      ...
      proof: this.generateMembershipProof(
        kp.getPublic().encode("hex", true),
        this.serverCommitment
      ),
    };
  }
  ...
  userAuth(proof, userinfo) {
    const tmpCommitment = crypto
      .createHash("sha256")
      .update(mystery.key + "-" + userinfo.password + "-" + proof.timestamp)
      .digest("hex");
  ...
  generateMembershipProof(publicKey, groupCommitment) {
    ...
    const response = ec
      .keyFromPrivate(randomValue, "hex")
      .getPrivate()
      .toString(16);
  ...
  verifyMembershipProof(proof, groupCommitment, publicKey) {
    ...
    return (
      ...
      ec.keyFromPublic(publicKey, "hex").getPublic().encode("hex", true) ===
        commitment
    );
  }
}
